...
from src.monitoring.metrics import ACTIVE_STREAMS
...
async def check_active_streams(alert_service: AlertService) -> None:
    # Use the unified settings for maximum concurrent streams.
    while True:
        try:
            active_count = ACTIVE_STREAMS._value if hasattr(ACTIVE_STREAMS, "_value") else 0
            if active_count > settings.MAX_CONCURRENT_STREAMS:
                await alert_service.send_alert(
                    f"High number of active streams: {active_count}/{settings.MAX_CONCURRENT_STREAMS}"
                )
...
async def monitor_services(alert_service: AlertService):
    while True:
        await check_active_streams(alert_service)
        await asyncio.sleep(60)