...
class ActivityHeatmapWidget:
    template = """
    <div class="activity-heatmap-widget" data-widget-id="{id}">
        ...
        <canvas id="heatmap-{id}" class="heatmap-canvas"></canvas>
    ...
    async def get_activity_data(self, start_time: datetime, end_time: datetime) -> List[ActivityPoint]:
        # Fetch activity data from various sources
        user_activities = []  # Replace with actual data fetch
        
        # Process and normalize data
        max_value = max(a.value for a in user_activities)
        for activity in user_activities:
            activity.value = activity.value / max_value
